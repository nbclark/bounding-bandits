...
- (void)releaseBlocksOnMainThread
{
	...
	if (httpCompletionBlock) {
		[blocks addObject:httpCompletionBlock];
		[httpCompletionBlock release];
		httpCompletionBlock = nil;
	}
	if (failureBlock) {
...
- (void)setHTTPCompletionBlock:(void(^)(ASIHTTPRequest * request))aCompletionBlock
{
	[httpCompletionBlock release];
	httpCompletionBlock = [aCompletionBlock copy];
}

- (void)setFailedBlock:(void(^)(ASIHTTPRequest*))aFailedBlock
{
	[failureBlock release];
